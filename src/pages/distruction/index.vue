...
<div class="main-wrapper">
  ...
  <div class="distruction-wrapper">
    <img :src="require('./2-' + menuindex + '.png')" alt="" :class="{'bounceInUp animated' :  animate1}">
    <div class="text-wrapper" :class="'info-wrapper' + menuindex">
      <p v-if='menuindex != 0' class='info-1' :class="{'fadeInLeft animated' :  animate2}"><span>#CLOUDFOAM</span>ULTIMATE</p>
      <p v-if='texts[menuindex].summary' class='info-2' v-html='texts[menuindex].summary' :class="{'fadeInRight animated' :  animate3}"></p>
      <p class='info-3' v-html='texts[menuindex].label' :class="{'fadeInLeft animated' :  animate4}"></p>
    </div>
      ...
      wx.scanQRCode({
        ...
        success: function(resqr) {
          ...
          if (['mp4', 'MP4', 'mov', 'webm', 'ogg', 'jpg', 'gif', 'png', 'jpeg'].indexOf(item[item.length - 1]) > -1) {
            this.$router.push(`/pagek3wmenuk${item}wmediak${item[0]}wtypek${item[1]}`)
          } else {
